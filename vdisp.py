...
# particle data
starfilename = 'hdfs://sohnic:54310/data/TNG300/snap99/parquet/subhalos2ptls_star.parquet.snappy'
stardf = spark.read.parquet(starfilename)
dmfilename = 'hdfs://sohnic:54310/data/TNG300/snap99/parquet/subhalos2ptls_dm.parquet.snappy'
dmdf = spark.read.parquet(dmfilename)
...
def rotv_vdisp_aperture(df, aperture, ptl):
    ...
    if ptl == 'dm':
        filtered_df = filtered_df.withColumn("mass", F.lit(Mdm))
    ...
    # (5)
    rotv_vdisp_df = filtered_df.groupBy("subhalo_id").agg((F.sum("dispersion_weighted")*0.5).alias(f"total_kinetic_energy"))
    ...
    vdisp_x_df = filtered_df.groupBy("subhalo_id").agg(
        (F.sum(F.col("dispersion_x")*F.col("mass"))/F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_x"))
    rotv_vdisp_df = rotv_vdisp_df.join(vdisp_x_df, "subhalo_id")
    vdisp_y_df = filtered_df.groupBy("subhalo_id").agg(
        (F.sum(F.col("dispersion_y")*F.col("mass"))/F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_y"))
    rotv_vdisp_df = rotv_vdisp_df.join(vdisp_y_df, "subhalo_id")
    vdisp_z_df = filtered_df.groupBy("subhalo_id").agg(
        (F.sum(F.col("dispersion_z")*F.col("mass"))/F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_z"))
    rotv_vdisp_df = rotv_vdisp_df.join(vdisp_z_df, "subhalo_id")
    rotv_vdisp_df = rotv_vdisp_df.withColumn(f"mass_weighted_velocity_dispersion",
                            F.col(f"mass_weighted_velocity_dispersion_x")+F.col(f"mass_weighted_velocity_dispersion_y")+F.col(f"mass_weighted_velocity_dispersion_z"))
    ...
    Krot_groupdf = filtered_df.groupBy("subhalo_id").agg((F.sum("Krot")).alias(f"rotation_kinetic_energy"))
    rotv_vdisp_df = rotv_vdisp_df.join(Krot_groupdf, "subhalo_id")
    
    filtered_df = filtered_df.drop("Krot")

    # (15)
    rotv_groupdf = filtered_df.groupBy("subhalo_id").agg((F.sum("mV_rot") / F.max("mass_sum")).alias(f"mass_weighted_rotation_velocity"))
    ...
    vdisp_r_df = filtered_df.groupBy("subhalo_id").agg((F.sum(F.col("dispersion_r")*F.col("mass")) / F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_r"))
    rotv_vdisp_df = rotv_vdisp_df.join(vdisp_r_df, "subhalo_id")
    vdisp_theta_df = filtered_df.groupBy("subhalo_id").agg((F.sum(F.col("dispersion_theta")*F.col("mass")) / F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_theta"))
    rotv_vdisp_df = rotv_vdisp_df.join(vdisp_theta_df, "subhalo_id")
    vdisp_phi_df = filtered_df.groupBy("subhalo_id").agg((F.sum(F.col("dispersion_phi")*F.col("mass")) / F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_phi"))
    ...
    # (22)
    vdisp_phi_no_rotation_df = filtered_df.groupBy("subhalo_id").agg((F.sum((F.col("rel_vphi")-F.col(f"mass_weighted_rotation_velocity"))**2*F.col("mass"))/ F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_phi_no_rotation"))
    rotv_vdisp_df = rotv_vdisp_df.join(vdisp_phi_no_rotation_df, "subhalo_id")
    rotv_vdisp_df = rotv_vdisp_df.withColumn(f"mass_weighted_velocity_dispersion_no_rotation",
                                             F.col(f"mass_weighted_velocity_dispersion_r")+F.col(f"mass_weighted_velocity_dispersion_phi_no_rotation")+F.col(f"mass_weighted_velocity_dispersion_theta"))
    
    filtered_df = filtered_df.drop("rel_vphi")
    
    # (23)
    filtered_df = filtered_df.withColumn("dispersion_x_no_rotation", (F.col("rel_vx")-F.col(f"mass_weighted_rotation_velocity")*F.col("unitPhi_x"))**2)
    filtered_df = filtered_df.withColumn("dispersion_y_no_rotation", (F.col("rel_vy")-F.col(f"mass_weighted_rotation_velocity")*F.col("unitPhi_y"))**2)
    filtered_df = filtered_df.withColumn("dispersion_z_no_rotation", (F.col("rel_vz")-F.col(f"mass_weighted_rotation_velocity")*F.col("unitPhi_z"))**2)
    vdisp_x_df_no_rotation = filtered_df.groupBy("subhalo_id").agg(
        (F.sum(F.col("dispersion_x_no_rotation")*F.col("mass"))/F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_x_no_rotation"))
    ...
    vdisp_y_df_no_rotation = filtered_df.groupBy("subhalo_id").agg(
        (F.sum(F.col("dispersion_y_no_rotation")*F.col("mass"))/F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_y_no_rotation"))
    ...
    vdisp_z_df_no_rotation  = filtered_df.groupBy("subhalo_id").agg(
        (F.sum(F.col("dispersion_z_no_rotation")*F.col("mass"))/F.sum("mass")).alias(f"mass_weighted_velocity_dispersion_z_no_rotation"))
...
# function to calculate 3D stellar rotation velocity and stellar velocity dispersion based on particles within aper
def rotv_vdisp_column(df, column, ptl, scale=1):
    if ptl == 'dm':
        df = df.withColumn("mass", F.lit(Mdm))
        
    filtered_df = df.filter(F.col("sq_dist_subhalo2ptl") <= (scale*F.col(column)*h)**2)
...
for r in [1,3,5,10,20,30]:
    rotv_vdisp_stardf = rotv_vdisp_aperture(stardf, r, ptl='star').toPandas().sort_values(by="subhalo_id")
    rotv_vdisp_dmdf = rotv_vdisp_aperture(dmdf, r, ptl='dm').toPandas().sort_values(by="subhalo_id")

    os.makedirs('result/cat/3Dradius', exist_ok=True)
    os.makedirs(f'result/cat/3Dradius/{r}', exist_ok=True)

    star3d = rotv_vdisp_stardf[['mass_weighted_velocity_dispersion', 'mass_weighted_rotation_velocity', 'total_kinetic_energy',
                             'rotation_kinetic_energy', 'mass_weighted_velocity_dispersion_no_rotation', 'unitJ_x', 'unitJ_y', 'unitJ_z']]
    star2d = rotv_vdisp_stardf[['mass_weighted_velocity_dispersion_x', 'mass_weighted_velocity_dispersion_y',
                                'mass_weighted_velocity_dispersion_z', 'mass_weighted_velocity_dispersion_r',
                               'mass_weighted_velocity_dispersion_theta', 'mass_weighted_velocity_dispersion_phi',
                               'mass_weighted_velocity_dispersion_x_no_rotation', 'mass_weighted_velocity_dispersion_y_no_rotation',
                               'mass_weighted_velocity_dispersion_z_no_rotation', 'mass_weighted_velocity_dispersion_r_no_rotation',
                               'mass_weighted_velocity_dispersion_phi_no_rotation']]

    star3d = star3d.rename(columns={'mass_weighted_velocity_dispersion':'sigma2', 'mass_weighted_rotation_velocity':'Vrot',
                           'mass_weighted_velocity_dispersion_no_rotation':'sigma2_noRot', 'total_kinetic_energy':'Ktot',
                           'rotation_kinetic_energy':'Krot'})
    star2d = star2d.rename(columns={'mass_weighted_velocity_dispersion_x':'sigma2_x', 'mass_weighted_velocity_dispersion_y':'sigma2_y',
                           'mass_weighted_velocity_dispersion_z':'sigma2_z', 'mass_weighted_velocity_dispersion_r':'sigma2_r',
                           'mass_weighted_velocity_dispersion_theta':'sigma2_theta', 'mass_weighted_velocity_dispersion_phi':'sigma2_phi',
                           'mass_weighted_velocity_dispersion_x_no_rotation':'sigma2_x_noRot',
                           'mass_weighted_velocity_dispersion_y_no_rotation':'sigma2_y_noRot',
                           'mass_weighted_velocity_dispersion_z_no_rotation':'sigma2_z_noRot',
                           'mass_weighted_velocity_dispersion_r_no_rotation':'sigma2_r_noRot',
                           'mass_weighted_velocity_dispersion_phi_no_rotation':'sigma2_phi_noRot'})
    star3d.to_csv(f"result/cat/3Dradius/{r}/star3D.csv")
    star2d.to_csv(f"result/cat/3Dradius/{r}/star2D.csv")
    
    dm3d = rotv_vdisp_dmdf[['mass_weighted_velocity_dispersion', 'mass_weighted_rotation_velocity', 'total_kinetic_energy',
                             'rotation_kinetic_energy', 'mass_weighted_velocity_dispersion_no_rotation', 'unitJ_x', 'unitJ_y', 'unitJ_z']]
    dm2d = rotv_vdisp_dmdf[['mass_weighted_velocity_dispersion_x', 'mass_weighted_velocity_dispersion_y',
                                'mass_weighted_velocity_dispersion_z', 'mass_weighted_velocity_dispersion_r',
                               'mass_weighted_velocity_dispersion_theta', 'mass_weighted_velocity_dispersion_phi',
                               'mass_weighted_velocity_dispersion_x_no_rotation', 'mass_weighted_velocity_dispersion_y_no_rotation',
                               'mass_weighted_velocity_dispersion_z_no_rotation', 'mass_weighted_velocity_dispersion_r_no_rotation',
    ...
    dm3d = dm3d.rename(columns={'mass_weighted_velocity_dispersion':'sigma2', 'mass_weighted_rotation_velocity':'Vrot',
                           'mass_weighted_velocity_dispersion_no_rotation':'sigma2_noRot', 'total_kinetic_energy':'Ktot',
                           'rotation_kinetic_energy':'Krot'})
    dm2d = dm2d.rename(columns={'mass_weighted_velocity_dispersion_x':'sigma2_x', 'mass_weighted_velocity_dispersion_y':'sigma2_y',
                           'mass_weighted_velocity_dispersion_z':'sigma2_z', 'mass_weighted_velocity_dispersion_r':'sigma2_r',
                           'mass_weighted_velocity_dispersion_theta':'sigma2_theta', 'mass_weighted_velocity_dispersion_phi':'sigma2_phi',
                           'mass_weighted_velocity_dispersion_x_no_rotation':'sigma2_x_noRot',
                           'mass_weighted_velocity_dispersion_y_no_rotation':'sigma2_y_noRot',
                           ...
                           'mass_weighted_velocity_dispersion_r_no_rotation':'sigma2_r_noRot',
                           'mass_weighted_velocity_dispersion_phi_no_rotation':'sigma2_phi_noRot'})
    dm3d.to_csv(f"result/cat/3Dradius/{r}/dm3D.csv")
    dm2d.to_csv(f"result/cat/3Dradius/{r}/dm2D.csv")

for a in [1,2,3,5]:
    rotv_vdisp_stardf = rotv_vdisp_column(stardf, 'StarHalfRad', scale=a, ptl='star').toPandas().sort_values(by="subhalo_id")
    rotv_vdisp_dmdf = rotv_vdisp_column(dmdf, 'StarHalfRad', scale=a, ptl='dm').toPandas().sort_values(by="subhalo_id")

    os.makedirs('result/cat/3Dradius', exist_ok=True)
    os.makedirs(f'result/cat/3Dradius/{a}Rh', exist_ok=True)
    ...
    star2d = star2d.rename(columns={'mass_weighted_velocity_dispersion_x':'sigma2_x', 'mass_weighted_velocity_dispersion_y':'sigma2_y',
                           ...
                           'mass_weighted_velocity_dispersion_r_no_rotation':'sigma2_r_noRot',
                           'mass_weighted_velocity_dispersion_phi_no_rotation':'sigma2_phi_noRot'})
    star3d.to_csv(f"result/cat/3Dradius/{a}Rh/star3D.csv")
    star2d.to_csv(f"result/cat/3Dradius/{a}Rh/star2D.csv")
    ...
    dm2d = dm2d.rename(columns={'mass_weighted_velocity_dispersion_x':'sigma2_x', 'mass_weighted_velocity_dispersion_y':'sigma2_y',
                           ...
                           'mass_weighted_velocity_dispersion_r_no_rotation':'sigma2_r_noRot',
                           'mass_weighted_velocity_dispersion_phi_no_rotation':'sigma2_phi_noRot'})
    dm3d.to_csv(f"result/cat/3Dradius/{a}Rh/dm3D.csv")
    dm2d.to_csv(f"result/cat/3Dradius/{a}Rh/dm2D.csv")
