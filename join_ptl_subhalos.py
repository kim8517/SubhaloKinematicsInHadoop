...
# assign subbox number and filtering based on the radius for star particles
id_size = 6000
subbox_size = box_size/id_size
half_box = box_size/2

star_ptldf = star_ptldf.withColumn("ix", floor(F.col("px") / subbox_size ).cast('int'))
star_ptldf = star_ptldf.withColumn("iy", floor(F.col("py") / subbox_size ).cast('int'))
star_ptldf = star_ptldf.withColumn("iz", floor(F.col("pz") / subbox_size ).cast('int'))

dm_ptldf = dm_ptldf.withColumn("ix", floor(F.col("px") / subbox_size ).cast('int'))
dm_ptldf = dm_ptldf.withColumn("iy", floor(F.col("py") / subbox_size ).cast('int'))
dm_ptldf = dm_ptldf.withColumn("iz", floor(F.col("pz") / subbox_size ).cast('int'))
...
joined_star_df = joined_star_df.checkpoint()

# assign subbox number and filtering based on the radius for dm particles
boxnumber_joined_dm_ptldf = (
    dm_ptldf.alias("particles").join(
        broadcast_subdf.alias("subhalos"),
        (int_ptl2subhalo("particles.ix", "subhalos.subhalo_ix")  &
         int_ptl2subhalo("particles.iy", "subhalos.subhalo_iy")  &
         int_ptl2subhalo("particles.iz", "subhalos.subhalo_iz"))
...
boxnumber_joined_dm_ptldf = boxnumber_joined_dm_ptldf.withColumn("rel_px", F.pmod(F.col("particles.px") - F.col("subhalos.subhalo_px") + half_box, box_size) - half_box)
boxnumber_joined_dm_ptldf = boxnumber_joined_dm_ptldf.withColumn("rel_py", F.pmod(F.col("particles.py") - F.col("subhalos.subhalo_py") + half_box, box_size) - half_box)
boxnumber_joined_dm_ptldf = boxnumber_joined_dm_ptldf.withColumn("rel_pz", F.pmod(F.col("particles.pz") - F.col("subhalos.subhalo_pz") + half_box, box_size) - half_box)

boxnumber_joined_dm_ptldf = boxnumber_joined_dm_ptldf.withColumn("sq_dist_subhalo2ptl", F.pow(F.col("rel_px"), 2) + F.pow(F.col("rel_py"), 2) + F.pow(F.col("rel_pz"), 2))

sq_radius = (50*h)**2
joined_dm_df = boxnumber_joined_dm_ptldf.filter(F.col("sq_dist_subhalo2ptl")<sq_radius)
joined_dm_df = joined_dm_df.checkpoint()
...
filename = 'hdfs://sohnic:54310/data/TNG300/snap99/parquet/subhalos2ptls_star.parquet.snappy'
joined_star_df.write.option("compression", "snappy").mode("overwrite").parquet(filename)
filename = 'hdfs://sohnic:54310/data/TNG300/snap99/parquet/subhalos2ptls_dm.parquet.snappy'
joined_dm_df.write.option("compression", "snappy").mode("overwrite").parquet(filename)